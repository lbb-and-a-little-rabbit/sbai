...
double expectimax(const Board& board, int depth, bool maximizingPlayer) {
    if (depth == 0 || board.isTerminal())
        return board.evaluate();

    std::vector<Move> moves = board.generateMoves();

    if (maximizingPlayer) { // AI 走
        double maxValue = -std::numeric_limits<double>::infinity();
        for (const Move& move : moves) {
            double value = expectimax(board.applyMove(move), depth - 1, false);
            if (value > maxValue) maxValue = value;
        }
        return maxValue;
    } else { // 对手走，假设随机选择（期望节点）
        double sum = 0;
        double p = 1.0 / moves.size(); // 每个走法等概率
        for (const Move& move : moves) {
            sum += p * expectimax(board.applyMove(move), depth - 1, true);
        }
        return sum;
    }
}

// 为 AI 选择最佳走法（Expectimax 版本）
Move findBestMoveExpectimax(const Board& board, int depth) {
...
int main() {
    ...
    while (!board.isTerminal()) {
        ...
        // AI 走
        Move best = findBestMoveExpectimax(board, 3);
